...
def test_warns_when_both_key_and_enablement_token(patched_requests):
  with pytest.warns(UserWarning, match="An enablement token was provided, so I'm enabling the local API for you. If you've already enabled the local API on your board, remove the enablement token and try again."):
    b = vestaboard.Board(localApi={'enablementToken': 'token', 'ip': '10.0.0.1', 'key': 'fakeKey'})
    assert b.localIP == '10.0.0.1', "Local IP should be the passed-in IP"
    assert b.localKey == 'fakelocalkey', "Should use the new key from the board"
